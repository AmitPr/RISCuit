...
fn exec_op_at<E: std::error::Error>(
    ...
    // The sentinels dispatch through the same jump table as real ops; the
    // slow arm re-enters with the tree-decoded op.
    let mut op = op;
    loop {
        match op {
            Rv64IMASC::Invalid => fail!(HartError::invalid(pc, inst).into()),
            Rv64IMASC::Slow => match Rv64IMASC::parse_slow(inst) {
                Some(resolved) => {
                    op = resolved;
                    continue;
                }
                None => fail!(HartError::invalid(pc, inst).into()),
            },
            // --- RV64I ---
