...
use super::{mem_fault, Exec, Execute, Hart, X32};
use crate::{
...
fn exec_op_at<M: Memory<Addr = u32>, E: std::error::Error>(
    ...
    pc: u32,
    mem: &mut M,
) -> Result<Exec<u32>, MachineError<E>> {
    ...
    match op {
        ...
        Rv32IMASC::Unimp(_) => return Err(HartError::illegal(pc, inst).into()),
        ...
        Rv32IMASC::CUnimp(_) => return Err(HartError::illegal(pc, inst).into()),
    }
