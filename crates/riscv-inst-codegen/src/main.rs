...
fn main() {
    ...
    for (isa, code, c_table) in &codegen {
        ...
        if let Some(c_table) = c_table {
            let out_file = out_dir.join(format!("{isa}_lookup_table.rs"));
            let mut file = std::fs::File::create(&out_file).unwrap();
            file.write_all(c_table.as_bytes()).unwrap();
        }
    }
...
fn codegen() -> Vec<(String, String, Option<String>)> {
    ...
    gen_isas
        .into_iter()
        .map(|(isa, opcodes)| {
            let (table, isa_enum) = generate_isa_enum(isa, opcodes, &accessors);
...
fn generate_isa_enum(
    ...
    let (table, decode_fn) = generate_opcode_parser(&mut opcodes, isa);
    opcodes.sort_by(|a, b| {
