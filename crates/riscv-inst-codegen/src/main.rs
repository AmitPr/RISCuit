...
fn main() {
    ...
    for (isa, code, includes) in &codegen {
    ...
    let mod_code = {
        let mods = codegen.iter().map(|(isa, _, _)| {
...
fn codegen() -> Vec<(String, String, Vec<(String, String)>)> {
    ...
    gen_isas
        .into_iter()
        .map(|(isa, opcodes)| {
            let (table, table32, isa_enum) = generate_isa_enum(isa, opcodes, &accessors);
            ...
            if let Some(t) = table {
                includes.push((format!("{isa}_lookup_table.rs"), t.to_string()));
            }
            if let Some(t) = table32 {
                includes.push((format!("{isa}_lookup_table32.rs"), t.to_string()));
            }

            (isa.to_string(), prettyplease::unparse(&gen), includes)
        })
        .collect()
}
...
fn generate_operand_accessor_fn(operand: &[String]) -> TokenStream {
    ...
    let accessor = serialize_bitspecs(quote! { inst }, spec_ty, &operand[1]);

    quote! {
        #[inline(always)]
        pub const fn #fn_ident(&self, inst: u32) -> #operand_type {
            #accessor
        }
    }
}
...
fn generate_isa_enum(
    ...
    let (table, table32, decode_fn) = generate_opcode_parser(&mut opcodes, isa);
    opcodes.sort_by(|a, b| {
        if a.discriminant.map_or(u8::MAX, |x| x) < b.discriminant.unwrap_or(u8::MAX) {
